...
import 'package:soil_humidity_app/utils.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    bluetooth.onDeviceStatusChanged().listen((state) {
      print('Device status changed: $state');
      setState(() {
        btState = state;
        percentValue = 0;
      });
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              var granted = await askBluetoothPermission();
              if (!granted) return;
